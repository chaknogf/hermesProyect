...
from app.schemas.fuentes_externas import FuenteExternaCreate, FuenteExternaOut, FuenteExternaBase, FuentesExternas
...
@router.get("/fuentesExternas", response_model=List[FuentesExternas])
async def listar_fuentes(
    ...
    try:
        ...
        return result  # ← No uses JSONResponse aquí
...
async def obtener_fuente(
    ...
    try:
        ...
        if result is None:
            raise HTTPException(status_code=404, detail=f"Fuente externa con id {fuente_id} no encontrada.")
        return result
    except SQLAlchemyError as e:
        raise HTTPException(status_code=500, detail=str(e))
